...
$connor=mysqli_connect('localhost','root','deadbeef88','custom');//DECLARING CONNOR BECAUSE FOR SOME STUPID REASON conCouch is also $con (YOU SHOULD CHANGE THIS!!!)
$zes = mysqli_query($connor,"SELECT catsubcat FROM datasets WHERE id=$p");
while($zow = mysqli_fetch_array($zes)){ 
    $tempVal1 = $zow['catsubcat'];
    $fco=0;
...
while($aow = mysqli_fetch_array($aes)){ 
    ...
    while($xow = mysqli_fetch_array($xes)){ 
        ...
        if($match>0){
            //echo "match<br>";
            $final[$fincounter][0]=$match;
            $final[$fincounter][1]=$id;
            $fincounter++;
        }
    } 
}
...
if($pageid==''){
    ...
    while($aow = mysqli_fetch_array($aes)){ 
        ...
        if($value==$p||$value==$final[0][1]||$value==$final[1][1]||$value==$final[2][1]||$value==$final[3][1]||$value==$final[4][1]||$value==$final[5][1]){
            $donothing=1;
        } else{
            $temp++;
        }
...
if($type=='News'){
    $color='#ff6d00';
    $colgb='rgba(255,109,0,.9)';
    $colas='cnews';
} else if($type=='Exclusive'){
    $color='#3a98b4';
    $colgb='rgba(58,152,180,.9)';
    $colas='cexclusive';
} else if($type=='Visualization'){
    $color='#5ca41a';
    $colgb='rgba(92,164,26,.9)';
    $colas='cdata';
} else if($type=='Video'){
    $color='#157d9a';
    ...
    $color='#635c4c';
    $colgb='rgba(99,92,76,.9)';
    $colas='clife';
} else if($type=='Announcement'){
    $color='#445eb2';
    $colgb='rgba(68,94,178,.9)';
    $colas='cannouncement';
} else if($type=='Support'){
    $color='#c60033';
    $colgb='rgba(198,0,51,.9)';
    $colas='csupport';
} else if($type=='Quiz'){
    $color='#c60033';
    $colgb='rgba(198,0,51,.9)';
