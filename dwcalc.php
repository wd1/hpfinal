...
<div class='pinky'><div class='loadercss'></div></div>
<style type='text/css'>
...
function nextStep(){
  if(starter >= ender){//treat this as final function or just call a final function
    loc=loc.split('dw')[1];
    loc=loc.split('&yf=')[0];
    $.ajax({
    url:'inc/aaaa.php',
      type: 'POST',
      data: {loc:loc,realstart:realstart,ender:ender},
    }).done(function(data){
      $('.pinky').hide();
      window.location.href="upl/"+data+".gif";
      //window.open("upl/"+data+".gif","_blank");
      //location.reload();
    });
  ...
  starter++;
  html2canvas(x[starter], {
      onrendered: function (canvas) {
        $('#placy'+starter+'').html('');
        document.body.appendChild(canvas);
        var img = canvas.toDataURL('image/png');
        img=img.split('data:image/png;base64,')[1];
        var locker=lockray[starter];
