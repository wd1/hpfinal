  ...
  <div class='drawer'>
    ...
    while($aow = mysqli_fetch_array($aes)){ 
      if($_GET[cid]==$aow[id]){//IF IT IS THIS TOPIC ON THE LIST OF TOPICS ON THE LEFT TURN IT ORANGE AND OPEN IT
        ...
        if($count==1){    
          echo "<a href='content?cid=".$aow['id']."' class='soody-on dl a".$aow['id']."' data-id='".$aow['id']."'>".$aow['cat']."</a>";  
        } else{
          echo "<a class='soody-on dl a".$aow['id']."' data-id='".$aow['id']."'>".$aow['cat']."</a>";  
...
if($_GET[sid]==0){
  ...
  while($aow = mysqli_fetch_array($aes)){ 
    ...
    //echo "<h1 data-cid='".$aow['id']."' class='hone'><a><section>".$aow['cat']."</section></h1>";
  ...
  while($aow = mysqli_fetch_array($aes)){ 
    echo "<div class='filta-wrap'><span class='filta'><i class='fa fa-filter'></i> Filter</span></div>";
    //echo "<h1 data-cid='".$_GET[cid]."' data-sid='".$_GET[sid]."' class='hone'>".$aow['cat']." <span>/</span> ".$aow['subcat']."<span class='filta'><i class='fa fa-filter'></i> Filter</span></h1>";
    echo "<style>.tiptop{margin-top:25px;}</style>";
  }   
}
...
$lord=$_GET[cid];
if($runs>1){
  include 'indexTopic.php';
  include 'indexTopic.php';  
} else{
  include 'indexTopic.php';
}

echo "<input class='cid' type='hidden' value='".$_GET[cid]."'>";
